    ...
    lobby_finder(io_context, config)
{
    ...
    std::pair<boost::filesystem::path, SoundType> collidePair { getRepoRoot() / "assets/sounds/collide.wav", SoundType::Collision };
    audioPaths.push_back(collidePair);

    std::vector< std::pair<boost::filesystem::path, SoundType>> musicPaths;
    std::pair<boost::filesystem::path, SoundType> pianoPair{ getRepoRoot() / "assets/sounds/piano.wav", SoundType::Background };
